...
#     def __init__(self, d_model, nhead, dropout=0.1, window_size=(7,7)):
#         super().__init__()
#         self.nhead = nhead
...
#     def __init__(self, d_model, nhead, window_size=(7,7), mlp_ratio=4.0, dropout=0.2):
#         super().__init__()
...
class RelativeSelfAttentionFusionAvg(nn.Module):
    ...
    def forward(self, features: List[torch.Tensor]) -> torch.Tensor:
        ...
        tokens = []
        for proj, feat in zip(self.projs, features):
            avg_pooled = F.adaptive_avg_pool2d(feat, self.patch_size)  # [B, C_i, patch_H, patch_W]
            max_pooled = F.adaptive_max_pool2d(feat, self.patch_size)
            ...
            token = proj_feat.view(B, C, H * W).transpose(1, 2)  # [B, H*W, out_channels]
            tokens.append(token)
        # 모든 stage의 토큰을 concat: total_tokens = num_stages * (patch_H * patch_W)
        token_seq = torch.cat(tokens, dim=1)  # [B, total_tokens, out_channels]
        for layer in self.layers:
            token_seq = layer(token_seq)
        fused_feature = token_seq.mean(dim=1)  # [B, out_channels]
